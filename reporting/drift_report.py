...
def generate_data_drift_report(ref_df: pd.DataFrame, prod_df: pd.DataFrame) -> Report:
    """
    Generate a comprehensive data drift report using Evidently 0.7.x.
...
def generate_classification_report(prod_df: pd.DataFrame) -> Report:
    ...
    # Rename columns for Evidently classification preset
    class_df = prod_df.rename(columns={'label': 'target'})
    class_dataset = Dataset.from_pandas(class_df)
    
    report = Report([ClassificationPreset()])
    report.run(current_data=class_dataset)
...
def get_drift_summary(ref_df: pd.DataFrame, prod_df: pd.DataFrame) -> dict:
    """
    Get a quick summary of drift status using sklearn-based comparison.
    Evidently 0.7.x doesn't provide easy programmatic drift metrics,
    so we calculate a simple feature-wise drift approximation.
    
    Returns:
        dict with drift statistics
    """
    # Simple drift detection based on mean/std comparison
    ...
    for col in FEATURE_COLUMNS:
        if col in ref_df.columns and col in prod_df.columns:
            ...
            # Check if production mean is more than 2 std away from reference
            if ref_std > 0 and abs(prod_mean - ref_mean) > 2 * ref_std:
                drifted_count += 1
    
    drift_share = drifted_count / n_features if n_features > 0 else 0
    
    return {
        'dataset_drift': drift_share > 0.3,  # >30% features drifted
